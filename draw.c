...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  ...
  double top[3],middle[3],bottom[3];
  int x,y,z,ycurrent;
  x = 0;
  y = 1;
  z = 2;
  ...
  c.green = rand() % 255;
  c.blue = rand() % 255;
  double x0,x1,z0,z1,dx0,dx1,dx1_,dz0,dz1,dz1_;
  if (points->m[y][i] >= points->m[y][i + 1]){
    if (points->m[y][i] >= points->m[y][i + 2]){
      top[x] = points->m[x][i];
      top[y] = points->m[y][i];
      top[z] = points->m[z][i];
      if (points->m[y][i + 1] >= points->m[y][i + 2]){
	middle[x] = points->m[x][i + 1];
	middle[y] = points->m[y][i + 1];
	middle[z] = points->m[z][i + 1];
	bottom[x] = points->m[x][i + 2];
	bottom[y] = points->m[y][i + 2];
	bottom[z] = points->m[z][i + 2];
      }
      else{
	middle[x] = points->m[x][i + 2];
	middle[y] = points->m[y][i + 2];
	middle[z] = points->m[z][i + 2];
	bottom[x] = points->m[x][i + 1];
	bottom[y] = points->m[y][i + 1];
	bottom[z] = points->m[z][i + 1];
      } 
    }
    else{
      top[x] = points->m[x][i + 2];
      top[y] = points->m[y][i + 2];
      top[z] = points->m[z][i + 2];
      middle[x] = points->m[x][i];
      middle[y] = points->m[y][i];
      middle[z] = points->m[z][i];
      bottom[x] = points->m[x][i + 1];
      bottom[y] = points->m[y][i + 1];
      bottom[z] = points->m[z][i + 1];
    }
    ...
    if (points->m[y][i + 1] >= points->m[y][i + 2]){
      top[x] = points->m[x][i + 1];
      top[y] = points->m[y][i + 1];
      top[z] = points->m[z][i + 1];
      if (points->m[y][i] >= points->m[y][i + 2]){
	middle[x] = points->m[x][i];
	middle[y] = points->m[y][i];
	middle[z] = points->m[z][i];
	...
	middle[y] = points->m[y][i + 2];
	middle[z] = points->m[z][i + 2];
	bottom[x] = points->m[x][i];
	bottom[y] = points->m[y][i];
	bottom[z] = points->m[z][i];
      }
    }
    else {
      top[x] = points->m[x][i + 2];
  ...
  z0 = bottom[z];
  z1 = bottom[z];
  dx0 = (top[x] - bottom[x]) /(top[y] - bottom[y] + 1);
  dx1 = (middle[x] - bottom[x]) / (middle[y] - bottom[y] + 1);
  dx1_ = (top[x] - middle[x]) / (top[y] - middle[y] + 1);
  dz0 = (top[z] - bottom[z]) / (top[y] - bottom[y] + 1);
  dz1 = (middle[z] - bottom[z]) / (middle[y] - bottom[y] + 1);
  dz1_ = (top[z] - middle[z]) / (top[y] - middle[y] + 1);
  while (ycurrent <=  middle[y]){
    draw_line(x0,ycurrent,z0,x1,ycurrent,z1,s,zb,c);
    ycurrent+= 1;
    x0 += dx0;
    x1 += dx1;
    z0 += dz0;
